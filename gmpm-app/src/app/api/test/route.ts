...
// API de teste para validar TODOS os cálculos do sistema

import { NextResponse } from 'next/server';

// Simular dados para teste de cálculos
interface TestCase {
    name: string;
...
// 6. Teste de Position Sizing
function calculatePositionSize(score: number): { size: string; riskPercent: number } {
    if (score >= 80) return { size: '0.5R', riskPercent: 0.5 };
    if (score >= 70) return { size: '0.3R', riskPercent: 0.3 };
    return { size: '0.2R', riskPercent: 0.2 };
}

// 7. Teste de Regime Detection
...
export async function GET() {
    ...
    tests.push({
        ...
        expectedOutput: 104.5, // 100 + (3 * 1.5)
        actualOutput: tp1,
        passed: tp1 === 104.5,
    });
    tests.push({
        ...
        expectedOutput: 107.5, // 100 + (3 * 2.5)
        actualOutput: tp2,
        passed: tp2 === 107.5,
    });
    ...
    const regimes = [
        ...
        { vix: 20, fg: 50, yc: -0.2, expected: 'RISK_OFF' }, // Inverted curve
    ];
